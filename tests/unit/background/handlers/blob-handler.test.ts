  ...
  describe('handleBlobUrl', () => {
    describe('Normal Cases', () => {
      it('should successfully register blob URL with all parameters', () => {
        ...
        const message = {
          blobUrl: 'blob:https://facebook.com/12345678-1234-1234-1234-123456789abc',
          blobType: 'audio/mpeg',
          blobSize: 64000,
          durationMs: 5000,
      ...
      it('should register blob URL without optional parameters', () => {
        ...
        const message = {
          blobUrl: 'blob:https://facebook.com/87654321-4321-4321-4321-987654321cba',
          durationMs: 3000,
      ...
      it('should log debug information with truncated blob URL', () => {
        ...
        const message = {
          blobUrl: 'blob:https://facebook.com/very-long-uuid-12345678-1234-1234-1234-123456789abcdef',
          blobType: 'audio/wav',
          blobSize: 128000,
          durationMs: 8000,
    ...
    describe('Validation and Error Handling', () => {
      it('should handle null voiceMessagesStore', () => {
        ...
        expect(result).toBe(true);
        expect(mockLogger.error).toHaveBeenCalledWith('voiceMessagesStore does not exist');
        expect(mockSendResponse).toHaveBeenCalledWith({
          success: false,
          message: 'Internal error: voiceMessagesStore does not exist'
        });
        expect(mockVoiceMessagesStore.registerDownloadUrl).not.toHaveBeenCalled();
      });
      ...
      it('should handle missing blobUrl', () => {
        ...
        expect(result).toBe(true);
        expect(mockLogger.error).toHaveBeenCalledWith('Missing required Blob URL or duration information');
        expect(mockSendResponse).toHaveBeenCalledWith({
          success: false,
  ...
  describe('handleBlobContent', () => {
    describe('Normal Cases', () => {
      it('should successfully download blob content with MP3 type', () => {
        ...
        (global.chrome as any).downloads.download.mockImplementation((options: any, callback: any) => {
          callback(mockDownloadId);
        });

        const base64Data = 'SUQzAwAAAAABUEZUIT0AAAA4AAAAL/////8AAABJRCMBAAAAAElEMwMAAAEAAAA=';
        const message = {
      ...
      it('should handle blob content without timestamp', () => {
        ...
        jest.spyOn(Date.prototype, 'toISOString').mockReturnValue('2023-10-31T12-00-00.000Z');
    ...
    describe('Validation and Error Handling', () => {
      it('should handle missing base64data', () => {
        ...
        expect(result).toBe(true);
        expect(mockLogger.error).toHaveBeenCalledWith('Missing required parameters');
        expect(mockSendResponse).toHaveBeenCalledWith({
          success: false,
          error: 'Missing required parameters'
        });
        expect((global.chrome as any).downloads.download).not.toHaveBeenCalled();
      });
    ...
    describe('Integration Tests', () => {
      it('should work with realistic blob content download workflow', () => {
        ...
        (global.chrome as any).downloads.download
          .mockImplementationOnce((options: any, callback: any) => callback(mockDownloadId1))
          .mockImplementationOnce((options: any, callback: any) => callback(mockDownloadId2));
        ...
        expect(result).toBe(true);
        expect((global.chrome as any).downloads.download).toHaveBeenCalledTimes(2);
        expect(mockSendResponse).toHaveBeenCalledWith({
          success: true,
          message: 'File download started',
